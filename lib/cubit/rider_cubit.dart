...
import '../featers/profile/screen/profile.dart';
...
class RiderCubit extends Cubit<MandoubState> {
  RiderCubit() : super(MandoubInitial()){_getCurrentLocation();}
  ...
  List bottomNavigationClasses = [const MainHome(),const OrderHandle(),LastOrder(),const Profile(),];
  ...
  void changePassType(icon,password){
    iconPassword =  icon==Icons.visibility_outlined ?  Icons.visibility_off_outlined : Icons.visibility_outlined;
  ...
  void riderLogin({
    ...
    emit(LoginLoading());
    DioHelper.postData(url: 'delivery-partners/login', data: {
  ...
  void getRiderData(){
    ...
    DioHelper.getData(url: 'delivery-partners/auth/me',
        token: token,
    ).then((value){
